...
for mouse_ind = 1:length(mouse_folders)
    ...
    for filenumber = 1:length(mouse_files)
        if strcmp(mouse_files(filenumber).name(end-3:end), '.mat') == 1 && strcmp(mouse_files(filenumber).name(1:4), 'whee') == 1
        else
    ...
    for file_ind = 1:length(mouse_files)
        ...
        orDiff = abs(abs(orientationL) - abs(orientationR)); 
        ...
        %%% ONLY TAKE SESSIONS >60%, that have contrast levels, 20 deg
        %%% difference and more than 100 trials in the session
        if mean(correct) > 0.55 && exist('contrastR','var') && trialLength > 100
