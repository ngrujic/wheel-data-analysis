...
for dayInd = 1:length(dayCell)
    ...
    try
        ...
        if pressed == 71
            ...
            [eyeCentre, pupilSize,eyesLikelihood,licks,eyePointDistances] = getPupilSizePositionCSV(csvDat);
...
%             dayDLC(:,3) = zscore(lowpass(dayDLC(:,3),20,60));
            ...
            dayDLC(:,12) = movmean(dayDLC(:,12),30);
            ...
            for onInd = 1:length(onsets)
                ...
                if min(likelihoodTrials(:)) > blinkLikelihoodThresh && sum(isnan(outlierTest))==0
                    ...
                    lick(goodTrialInd,:) = dayDLC(currentOnset - base:currentOnset + post,12);
