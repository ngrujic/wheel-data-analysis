%% Calculates the hits, misses, FAs, CRs and dprime for all types of trials individually
clear all
close all
...
targets = sort(unique(target_orient));
...
dprime = flip(dprime);
targets = flip(abs(targets));
hitRate = flip(hitRate);
faRate = flip(faRate);

bar(targets-5,hitRate,0.2,'g');hold on;bar(targets+5,faRate,0.2,'r')
xticks([0,45,90]);ylim([0 1])